...
seed="0" #0,1956 ,1383,5123,2494,3420,4123,6231,87,9913
device="gpu"
...
lambda_dx="1e-3" # Consistency 1e-4
lambda_dz="1e-3" # Model approximation 1e-4 
...
OUTPUT_PREFIX=${problem}_${latent_dim}_${net}_exDL${extraD_L}_exDM${extraD_M}_xs${xi_scale}_ly${layers}_wd${width}_Awd1${AE_width1}_Awd2${AE_width2}_${data_type}_${lambda_r_AE}_${lambda_jac_AE}_${lambda_dx}_${lambda_dz}_${lam}_lr${lr}_lrAE${lr_AE}_gam${gamma_lr}_WDGF${weight_decay_GFINNs}_${activation}_${activation_AE}_${seed}_${total_iteration}


python main_VC_tLaSDI.py --seed ${seed} --device ${device} --data_type ${data_type} --latent_dim ${latent_dim} --extraD_L ${extraD_L} --extraD_M ${extraD_M} --xi_scale ${xi_scale} --layers ${layers} --width ${width} --AE_width1 ${AE_width1} --AE_width2 ${AE_width2} --net ${net} --iterations ${iterations} --lambda_r_AE ${lambda_r_AE} --lambda_jac_AE ${lambda_jac_AE} --lambda_dx ${lambda_dx} --lambda_dz ${lambda_dz} --lr ${lr} --lr_AE ${lr_AE}  --gamma_lr ${gamma_lr} --miles_lr ${miles_lr} --weight_decay_AE ${weight_decay_AE} --weight_decay_GFINNs ${weight_decay_GFINNs} --activation ${activation} --activation_AE ${activation_AE} --load_model ${load_model} --load_iterations ${load_iterations} --lam ${lam} > ${OUTPUT_PREFIX}.log
