...
from utilities.plot import plot_latent_dynamics
...
#         self.num_para = 64
        ...
        if self.sys_name == 'viscoelastic' or self.sys_name == 'GC':
            if args.dtype == 'double':
                ...
                self.SAE = AutoEncoder(layer_vec_SAE, args.activation_SAE).double()

            elif args.dtype == 'float':
                self.SAE = AutoEncoder(layer_vec_SAE, args.activation_SAE).float()
...
#         while (epoch <= self.max_epoch):
        ...
        for epoch in tqdm(range(1, int(self.max_epoch) + 1), desc="Epoch Progress"):
    ...
    def detect_dimensionality(self):
        ...
        if self.sys_name == 'viscoelastic':
            ...
            if (self.save_plots == True):
                ...
                plot_latent_dynamics(x, self.dataset.dt, plot_name, self.output_dir)
