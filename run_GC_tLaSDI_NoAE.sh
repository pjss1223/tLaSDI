...
OUTPUT_PREFIX=${problem}_${latent_dim}_${net}_exDL${extraD_L}_exDM${extraD_M}_xs${xi_scale}_${lambda_r_SAE}_${lambda_jac_SAE}_${lambda_dx}_${lambda_dz}_${lam}_gamma${gamma_lr}_lr${lr}_${activation}_${iterations}_batch${batch_size}_seed${seed}


python main_GC_tLaSDI_NoAE.py --seed ${seed} --latent_dim ${latent_dim} --extraD_L ${extraD_L} --extraD_M ${extraD_M} --xi_scale ${xi_scale} --net ${net} --iterations ${iterations} --lambda_r_SAE ${lambda_r_SAE} --lambda_jac_SAE ${lambda_jac_SAE} --lambda_dx ${lambda_dx} --lambda_dz ${lambda_dz} --activation ${activation} --activation_SAE ${activation_SAE} --gamma_lr ${gamma_lr} --load_model ${load_model} --load_iterations ${load_iterations} --lam ${lam} --lr ${lr} --batch_size ${batch_size} > ${OUTPUT_PREFIX}.log
