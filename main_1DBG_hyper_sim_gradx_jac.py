...
def main(args):
    ...
    # p = 0.8
    problem = 'BG'
    AE_name = 'AE10hyper_sim_gradx_jac10000'
    # t_terminal = 40
    # dt = 0.1
    # trajs = 100
    order = 2
...
#(self,num_sensor, ind, extraD, depth_trunk=2, width_trunk=50, depth_hyper=2, width_hyper=50, act_trunk='relu', act_hyper='relu', initializer='default',softmax=False):
    ...
    # print(sum(p.numel() for p in neE.parameters() if p.requires_grad))
    ...
    ln.Brain_hyper_sim_gradx_jac.Init(**args2)
    ln.Brain_hyper_sim_gradx_jac.Run()
    ln.Brain_hyper_sim_gradx_jac.Restore()
    ln.Brain_hyper_sim_gradx_jac.Output()
    ln.Brain_hyper_sim_gradx_jac.Test()
