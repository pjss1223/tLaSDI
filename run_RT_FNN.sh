...
python main_RT_FNN.py --latent_dim ${latent_dim} --net ${net} --iterations ${iterations} --lambda_r_SAE ${lambda_r_SAE} --lambda_jac_SAE ${lambda_jac_SAE} --lambda_dx ${lambda_dx} --lambda_dz ${lambda_dz} --load_model ${load_model} --weight_decay_GFINNs ${weight_decay_GFINNs} --weight_decay_AE ${weight_decay_AE} --load_iterations ${load_iterations} > ${OUTPUT_PREFIX}.log
