...
lambda_dx="0" # Consistency 1e-4 1e-3
lambda_dz="0" # Model approximation 1e-4 1e-2
lam="1e-2"
...
OUTPUT_PREFIX=${problem}_${latent_dim}_${net}_${lambda_r_SAE}_${lambda_jac_SAE}_${lambda_dx}_${lambda_dz}_${lam}_${epochs}
...
python main_1DBG_tLaSDI_greedy.py --latent_dim ${latent_dim} --net ${net} --epochs ${epochs} --lambda_r_SAE ${lambda_r_SAE} --lambda_jac_SAE ${lambda_jac_SAE} --lambda_dx ${lambda_dx} --lambda_dz ${lambda_dz} --load_model ${load_model} --load_epochs ${load_epochs} --lam ${lam} > ${OUTPUT_PREFIX}.log
