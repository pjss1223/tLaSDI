...
def main(args):
    ...
    if load_model:
        AE_name = 'AE_nopara_sep'+ str(latent_dim)+'_extraD_'+str( extraD_L) +DI_str+ '_REC'+"{:.0e}".format(lambda_r_SAE)  + '_JAC'+ "{:.0e}".format(lambda_jac_SAE) + '_CON'+"{:.0e}".format(lambda_dx) + '_APP' + "{:.0e}".format(lambda_dz) + '_iter'+str(epochs+load_epochs)
    else:
        AE_name = 'AE_nopara_sep'+ str(latent_dim)+'_extraD_'+str( extraD_L) +DI_str+ '_REC'+"{:.0e}".format(lambda_r_SAE)  + '_JAC'+ "{:.0e}".format(lambda_jac_SAE) + '_CON'+"{:.0e}".format(lambda_dx) + '_APP' + "{:.0e}".format(lambda_dz) + '_iter'+str(epochs)
    ...
    ln.Brain_tLaSDI_para_sep.Init(**args2)
    ln.Brain_tLaSDI_para_sep.Run()
    ln.Brain_tLaSDI_para_sep.Restore()
    ln.Brain_tLaSDI_para_sep.Output()
    ln.Brain_tLaSDI_para_sep.Test()
