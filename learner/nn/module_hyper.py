...
def requr(input):
    ...
    '''
    return torch.relu(input ** 2) - torch.relu((
                                                           input - 0.5) ** 2)  # use torch.sigmoid to make sure that we created the most efficient implemetation based on builtin PyTorch functions
...
class Module_hyper(torch.nn.Module):
    '''Standard module format.
    '''

    def __init__(self):
        super(Module_hyper, self).__init__()
    ...
    def device(self, d):
        if d == 'cpu':
            ...
            for module in self.modules():
                if isinstance(module, Module_hyper):
...
class StructureNN_hyper(Module_hyper):
    '''Structure-oriented neural network used as a general map based on designing architecture.
    '''

    def __init__(self):
        super(StructureNN_hyper, self).__init__()

    def predict(self, x,mu, returnnp=False):
        if not isinstance(x, torch.Tensor):
            x = torch.tensor(x, dtype=self.dtype, device=self.device)
        return self(x,mu).cpu().detach().numpy() if returnnp else self(x,mu)


class LossNN_hyper(Module_hyper, abc.ABC):
    '''Loss-oriented neural network used as an algorithm based on designing loss.
    '''

    def __init__(self):
        super(LossNN_hyper, self).__init__()
