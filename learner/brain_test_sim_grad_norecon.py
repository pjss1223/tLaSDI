...
class Brain_test_sim_grad_norecon:
    ...
    def run(self):
        ...
        for i in range(self.iterations + 1):
           ...
           # z_gt_tt_norm1 = self.dataset.z[self.test_snaps[:-1] + 1, :]
            ...
            dx_data_train = J_e @ dz_gt_tr_norm
            ...
            #loss_AE_jac = self.SAE.jacobian_norm(z_gt_tr_norm, x)
           # loss_AE_GFINNs = torch.mean((z_sae_gfinns_tr_norm - z_gt_tr_norm1) ** 2)

            loss = loss_GFINNs+self.lambda_dx*loss_dx+self.lambda_dz*loss_dz #self.lambda_r*loss_AE_recon
            ...
            if i % self.print_every == 0 or i == self.iterations:
                ...
                dx_data_test = J_e @ dz_gr_tt_norm
               ...
               # loss_AE_recon_test = torch.mean((z_sae_tt_norm - z_gt_tt_norm) ** 2)
                ...
                loss_test = loss_GFINNs_test+loss_dx_test+loss_dz_test #+loss_AE_recon_test
                ...
                print(' ADAM || It: %05d, Loss: %.4e, loss_GFINNs: %.4e, loss_dx: %.4e, loss_dz: %.4e, Test: %.4e' %
                      (i, loss.item(),loss_GFINNs.item(),loss_dx.item(),loss_dz.item(), loss_test.item())) #,loss_AE_recon.item()
        ...
        return self.loss_history, self.loss_GFINNs_history, self.loss_AE_recon_history,self.loss_dx_history, data
