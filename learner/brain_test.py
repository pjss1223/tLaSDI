...
class Brain_test:
    ...
    def Init(cls, data, net, x_trunc, latent_idx, dt, z_gt, sys_name, output_dir, save_plots, criterion, optimizer, lr,
             iterations, lbfgs_steps,AE_name, path=None, batch_size=None,
        ...
        cls.brain = cls(data, net, x_trunc, latent_idx, dt, z_gt, sys_name, output_dir, save_plots, criterion,
                         optimizer, lr, weight_decay, iterations, lbfgs_steps, AE_name, path, batch_size,
    ...
    def __init__(self, data, net, x_trunc,latent_idx, dt,z_gt,sys_name, output_dir,save_plots, criterion, optimizer, lr, weight_decay, iterations, lbfgs_steps,AE_name, path, batch_size,
                 batch_size_test, print_every, save, callback, dtype, device):
        self.data = data
        self.net = net
    ...
    def output(self, data, best_model, loss_history, info, **kwargs):
        ...
        if loss_history:
            ...
            plt.yscale('log')
            plt.savefig(path + '/loss_AE_'+self.AE_name+self.sys_name+'.png')
            plt.show()
        if info is not None:
    ...
    def test(self, SAE, latent_idx):
        ...
        # Detruncate
        x_gfinn = torch.zeros([self.dim_t, SAE.dim_latent]).double()
        ...
        if (self.save_plots):
            ...
            if self.sys_name == 'viscoelastic':
                ...
                if (self.save_plots == True):
                    ...
                    plot_latent_visco(x_gfinn, self.dt, plot_name, self.output_dir)
                ...
                if (self.save_plots == True):
                    ...
                    plot_latent_tire(x_q, x_v, x_sigma, self.dt, plot_name, self.output_dir)
