...
#        self.latent_idx = latent_idx
    ...
    def run(self):
        ...
        for i in range(self.iterations + 1):
           ...
           # z_gt_tt_norm1 = self.dataset.z[self.test_snaps[:-1] + 1, :]
            ...
            # # print(dz_gt_tr.shape)
            # # print(grad(self.SAE.encode(z_gt_tr),z_gt_tr).shape)
            # dx_data_train = grad(self.SAE.encode(z_gt_tr_norm),z_gt_tr_norm) @ dz_gt_tr_norm
            ...
            if self.device == 'cpu':
                loss_AE_jac,_,_,_ = self.SAE.jacobian_norm_trunc(z_gt_tr_norm,x)
            else:
                loss_AE_jac, _, _, _ = self.SAE.jacobian_norm_trunc_gpu(z_gt_tr_norm, x)

            # loss_dx = torch.mean((dx_train - dx_data_train) ** 2)
            # loss_dz = torch.mean((dz_train - dz_gt_tr_norm) ** 2)
            #loss_dz =


            #loss_AE_jac,_,_,_  = self.SAE.jacobian_norm(z_gt_tr_norm, x)
           # loss_AE_GFINNs = torch.mean((z_sae_gfinns_tr_norm - z_gt_tr_norm1) ** 2)
            ...
            if i % self.print_every == 0 or i == self.iterations:
                ...
                if self.device == 'cpu':
                    loss_AE_jac_test,_,_,_ = self.SAE.jacobian_norm_trunc(z_gt_tt_norm,x_tt)
                else:
                    loss_AE_jac_test, _, _, _ = self.SAE.jacobian_norm_trunc_gpu(z_gt_tt_norm, x_tt)

                # loss_dx_test = torch.mean((dx_test - dx_data_test) ** 2)
                # loss_dz_test = torch.mean((dz_test - dz_gt_tt) ** 2)
                ...
                print(' ADAM || It: %05d, Loss: %.4e, loss_GFINNs: %.4e, loss_AE_recon: %.4e, loss_AE_jac: %.4e, Test: %.4e' %
                      (i, loss.item(),loss_GFINNs.item(),loss_AE_recon.item(),loss_AE_jac.item(), loss_test.item()))
        ...
        if self.sys_name == 'viscoelastic':
            ...
            # Plot latent variables
            if (self.save_plots == True):
                plot_name = '[Rolling Tire] SAE Latent Variables_' + self.AE_name
                plot_latent_tire(x_q, x_v, x_sigma, self.dataset.dt, plot_name, self.output_dir)

        # print('Done!', flush=True)
        return self.loss_history, self.loss_GFINNs_history, self.loss_AE_recon_history,self.loss_AE_jac_history, data
    ...
    def test(self):
        ...
        #z_net = torch.zeros(self.z_gt.shape)
