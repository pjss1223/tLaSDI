...
class Brain_hyper_sim_gradx_greedy:
    ...
    def Init(cls,  net, dt, sys_name, output_dir, save_plots, criterion, optimizer, lr,
             ...
             device='cpu',tol = 1e-3, tol2 = 2, adaptive = 'reg_max',n_train_max = 30,subset_size_max=80):
        cls.brain = cls( net, dt, sys_name, output_dir, save_plots, criterion,
                         ...
                         batch_size_test, print_every, save, callback, dtype, device, tol, tol2,adaptive,n_train_max,subset_size_max)
...
#        self.latent_idx = latent_idx
        ...
        if self.sys_name == '1DBurgers':
            ...
            # amp_train = np.linspace(0.7, 0.9, 2)
            # width_train = np.linspace(0.9, 1.1, 2)
            # amp_test = np.linspace(0.7, 0.9, 16)
            # width_test = np.linspace(0.9, 1.1, 16)
            #
    ...
    def run(self):
        ...
        for i in range(self.iterations + 1):
            ...
            # print(X_train.shape) # 4800 10
            # print(dx_train.shape) # 4800 10
            ...
            loss = loss_GFINNs+self.lambda_r*loss_AE+ self.lambda_dx*loss_dx #+loss_AE_GFINNs
            ...
            if i % self.print_every == 0 or i == self.iterations:
                ...
                for i_train in range(num_train):
                    ...
                    x0_train_tmp = x0_train_tmp.unsqueeze(0)
                    mu_tmp = mu_tmp.unsqueeze(0)

                    for snapshot in range(self.dim_t - 1):
                        x1_train_tmp = self.net.integrator2(self.net(x0_train_tmp), mu_tmp)
                ...
                # print(loss_AE)
                # print(err_max)
                print(' ADAM || It: %05d, Loss: %.4e, loss_GFINNs: %.4e, loss_AE_recon: %.4e, loss_dx: %.4e, validation test: %.4e' %
                    (i, loss.item(), loss_GFINNs.item(), loss_AE.item(), loss_dx.item(), err_max))
        ...
        return self.loss_history, self.loss_GFINNs_history, self.loss_AE_history, self.loss_dx_history, data
    ...
    def output(self, best_model, loss_history, info, **kwargs):
        ...
        if loss_history:
            ...
            p3,=plt.plot(self.loss_GFINNs_history[:,0], self.loss_GFINNs_history[:,1],'-')
            #p4,=plt.plot(self.loss_GFINNs_history[:,0], self.loss_GFINNs_history[:,2],'--')
            plt.legend(['train loss (GFINNs)'])  # , '$\hat{u}$'])
            plt.yscale('log')
            ...
            plt.legend(['train loss (AE)'])  # , '$\hat{u}$'])
            plt.yscale('log')
            ...
            plt.legend(['train loss (dx)'])  # , '$\hat{u}$'])
            plt.yscale('log')
