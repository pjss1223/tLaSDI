...
class Brain_test_sim_grad_noint:
    ...
    def run(self):
        ...
        for i in range(self.iterations + 1):
           ...
           # loss_AE_GFINNs = torch.mean((z_sae_gfinns_tr_norm - z_gt_tr_norm1) ** 2)
            ...
            loss = self.lambda_r*loss_AE_recon+self.lambda_dx*loss_dx+self.lambda_dz*loss_dz
            ...
            if i % self.print_every == 0 or i == self.iterations:
                ...
                loss_test = loss_AE_recon_test+loss_dx_test+loss_dz_test

                # print('{:<9}a loss: %.4e{:<25}Test loss: %.4e{:<25}'.format(i, loss.item(), loss_test.item()), flush=True)
                print(' ADAM || It: %05d, Loss: %.4e, loss_AE_recon: %.4e, loss_dx: %.4e, loss_dz: %.4e, Test: %.4e' %
                      (i, loss.item(),loss_AE_recon.item(),loss_dx.item(),loss_dz.item(), loss_test.item()))
        ...
        return self.loss_history, self.loss_AE_recon_history,self.loss_dx_history, data
