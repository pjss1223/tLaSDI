...
class Brain_hyper_sim_gradx_jac:
...
#        self.latent_idx = latent_idx
    ...
    def run(self):
        ...
        for i in range(self.iterations + 1):
            ...
            #print(self.net.consistency_loss(self.net(X_train), mu_train)) #check the cons loss

            #J_e, J_d,idx_trunc = self.SAE.jacobian_norm_trunc_wo_jac_loss(z_gt_tr_norm, x)
            if self.device == 'cpu':
                loss_AE_jac, J_e, J_d,idx_trunc = self.SAE.jacobian_norm_trunc(z_gt_tr_norm,x)
            else:
                loss_AE_jac, J_e, J_d,idx_trunc  = self.SAE.jacobian_norm_trunc_gpu(z_gt_tr_norm, x)
            ...
            loss = loss_GFINNs+self.lambda_r*loss_AE+self.lambda_dx*loss_dx+self.lambda_jac*loss_AE_jac
            ...
            if i % self.print_every == 0 or i == self.iterations:
                ...
                dx_test = self.net.f(X_test,mu_test)


                #J_e, J_d, idx_trunc = self.SAE.jacobian_norm_trunc_wo_jac_loss(z_gt_tt_norm, x_tt)
                if self.device == 'cpu':
                    loss_AE_jac_test, J_e, J_d, idx_trunc  = self.SAE.jacobian_norm_trunc(z_gt_tt_norm,x_tt)
                else:
                    loss_AE_jac_test, J_e, J_d, idx_trunc  = self.SAE.jacobian_norm_trunc_gpu(z_gt_tt_norm, x_tt)
                ...
                loss_test = loss_GFINNs_test+self.lambda_r*loss_AE_test+self.lambda_dx*loss_dx_test+self.lambda_jac*loss_AE_jac_test

                # print('{:<9}a loss: %.4e{:<25}Test loss: %.4e{:<25}'.format(i, loss.item(), loss_test.item()), flush=True)
                print(' ADAM || It: %05d, Loss: %.4e, loss_GFINNs: %.4e, loss_AE_recon: %.4e, loss_dx: %.4e, loss_JAC: %.4e, Test: %.4e' %
                    (i, loss.item(), loss_GFINNs.item(), loss_AE.item(), loss_dx.item(), loss_AE_jac.item(),loss_test.item()))
        ...
        return self.loss_history, self.loss_GFINNs_history, self.loss_AE_history, self.loss_dx_history, self.loss_AE_jac_history, data
    ...
    def output(self, best_model, loss_history, info, **kwargs):
        ...
        if loss_history:
            ...
            p8,=plt.plot(self.loss_dx_history[:,0], self.loss_dx_history[:,2],'--')
