...
def plot_latent(x_spnn, x_trunc, dEdt, dSdt, dt, plot_name, output_dir, sys_name):
    ...
    if (sys_name == 'viscoelastic'):
        plot_name = '[Viscoelastic] ' + plot_name
    elif (sys_name == 'rolling_tire'):
        plot_name = '[Rolling Tire] ' + plot_name
...
def plot_results(z_net, z_gt, dt, name, output_dir, sys_name):
    ...
    if (sys_name == 'viscoelastic'):
        ...
        # ax1 = axes.flatten()
        plot_name = '[GFINNs_BG] ' + name
        fig.suptitle(plot_name)

        ax1.plot(t_vec, u_net[:, nodes].detach(), 'b')
        ax1.plot(t_vec, u_gt[:, nodes].detach(), 'k--')
        l1, = ax1.plot([], [], 'k--')
        l2, = ax1.plot([], [], 'b')
        ax1.legend((l1, l2), ('GT', 'Net'))
        ax1.set_ylabel('$u$ [-]')
        ...
        plot_name = '[GFINNs_RT] ' + name + ' (Position and Velocity)'
        fig.suptitle(plot_name)
        ...
        fig, axes = plt.subplots(2, 3, figsize=(20, 10))
        ax1, ax2, ax3, ax4, ax5, ax6 = axes.flatten()
        plot_name = '[GFINNs_RT] ' + name + ' (Stress Tensor)'
        fig.suptitle(plot_name)
